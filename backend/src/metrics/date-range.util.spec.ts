...
describe('bogotaDayRangeToUtc', () => {
  it('converts a single-day Bogota range into UTC boundaries offset by 5 hours', () => {
    const { start, endExclusive } = bogotaDayRangeToUtc(
      '2026-07-15',
      '2026-07-15',
    );
  ...
  it('classifies an 8pm Bogota interaction as belonging to that Bogota day, not the next UTC day', () => {
    ...
    expect(
      eveningInteractionUtc >= start && eveningInteractionUtc < endExclusive,
    ).toBe(true);

    const { start: nextDayStart } = bogotaDayRangeToUtc(
      '2026-07-16',
      '2026-07-16',
    );
